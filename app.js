...
// ---- Hardcoded Content (text only — numbers come from Supabase) ----
const FACTS_DATA = [
  { icon: 'coffee',      title: '6-15 millioner ton',        description: 'Globalt produceres 6–15 mio. ton brugte kaffegrums om året.' },
  { icon: 'inbox',       title: '75% ender som affald',      description: 'Ca. 75% af brugte kaffegrums ender på lossepladsen.' },
  { icon: 'thermometer', title: 'Metan × 28',                description: 'Kaffegrums frigiver metan – 28 gange kraftigere end CO₂.' },
  { icon: 'info',        title: '30.000+ ton i Danmark',     description: 'Danmarks kaffeimport skaber over 30.000 ton kaffegrums årligt.' },
  { icon: 'flame',       title: '20-30% højere brændværdi',  description: 'Kaffegrumsbriketter brænder bedre end rene savsmuldsbriketter.' },
  { icon: 'info',        title: '100% affaldsmaterialer',    description: 'Alle råmaterialer er affald: kaffegrums, savsmuld, regnvand.' },
  { icon: 'info',        title: 'Født af udviklingsplanen',  description: 'Projektet udspringer af Valhallas udviklingsplan.' },
  { icon: 'tent',        title: 'Spejdernes Lejr 2026',     description: 'Projektet hænger sammen med SL 2026-temaet.' }
];

const BRANCHES_DATA = [
  { name: 'Mikrospejdere',  age_group: '6-8 år',   role_title: 'Regnvandssamlere',    description: 'De yngste spejdere indsamler regnvand til briketproduktionen. De lærer om vandets kredsløb og bæredygtighed.', icon: 'droplet',   level: 'Nybegynder' },
  { name: 'Minispejdere',   age_group: '8-10 år',  role_title: 'Solovns-eksperter',   description: 'Minispejderne bygger og bruger solovne til at tørre briketterne. De eksperimenterer med solenergi.',           icon: 'sun',       level: 'Nybegynder' },
  { name: 'Juniorspejdere',  age_group: '10-12 år', role_title: 'Briketpressere',      description: 'Juniorerne presser og former briketterne. De er produktionsholdets kerne og tester forskellige opskrifter.',    icon: 'press',     level: 'Nybegynder' },
  { name: 'Tropsspejdere',  age_group: '12-16 år', role_title: 'Forskere & testere',  description: 'Tropsmedlemmerne forsker i materialerne, måler brændværdi og dokumenterer forsøg. De samarbejder med Aarhus Universitet.', icon: 'flask', level: 'Nybegynder' },
  { name: 'Klanmedlemmer',  age_group: '16-25 år', role_title: 'Koordinering & PR',   description: 'Klanen koordinerer projektet, styrer hjemmesiden, kommunikation og samarbejdet med partnere og fonde.',        icon: 'megaphone', level: 'Nybegynder' }
];

const PARTNERS_DATA = [
  { name: 'BIPED / Aarhus Kommune', role: 'EU-projektpartner',       description: 'EU Horizon Europe-projekt: Positive Energy Districts i Brabrand' },
  { name: 'Aarhus Universitet',     role: 'Forskningspartner',       description: 'Studiegruppe der skriver afhandling om projektet' },
  { name: 'Brabrand Fineringscentral', role: 'Råmaterialeleverandør', description: 'Lokal leverandør af savsmuld – affald bliver råmateriale' },
  { name: 'Valhalla Gruppe / DDS',  role: 'Projektgruppe',           description: 'Spejdergruppen bag projektet – De Danske Spejdere i Brabrand' },
  { name: 'Inspirationspartnere',   role: 'Inspiration',             description: 'Grounded Cups, ØNSK Kaffesymbiose m.fl.' }
];

const TIMELINE_DATA = [
  { event_date: 'Februar 2026',  title: 'Godkendelse på ASE-møde',        description: 'Projektet blev officielt godkendt af ledelsen.',               completed: true },
  { event_date: 'Marts 2026',    title: 'Fondsansøgninger & BIPED-møde',  description: 'Ansøgninger til fonde og opstartsmøde med BIPED og AU.',       completed: false },
  { event_date: 'April 2026',    title: 'Første Valhalla-briket!',         description: 'Vi presser, tørrer og tester vores første rigtige briket.',    completed: false },
  { event_date: 'Maj–juni 2026', title: 'Fuld produktion',                 description: 'Alle grene er i gang – vi producerer mod målet.',              completed: false },
  { event_date: 'Juni 2026',     title: 'Sankt Hans med egne briketter',   description: 'Valhalla fejrer Sankt Hans med egne briketter!',               completed: false },
  { event_date: 'Juli 2026',     title: 'Spejdernes Lejr 2026',            description: 'Vi præsenterer projektet på SL2026.',                          completed: false }
];
...
async function loadAllData() {
  try {
    // Only fetch dynamic numbers from Supabase (metrics, branch counts, news, settings)
    const [metrics, branchRows, news, settingsRows] = await Promise.all([
    ...
    // Merge dynamic numbers into hardcoded branch data
    const branches = BRANCHES_DATA.map((b, i) => {
      const dbRow = branchRows && branchRows[i] ? branchRows[i] : {};
      return {
        ...b,
        scouts_count: dbRow.scouts_count || 0,
        briquettes_count: dbRow.briquettes_count || 0
      };
    });
    ...
    state.facts = FACTS_DATA;
    state.partners = PARTNERS_DATA;
